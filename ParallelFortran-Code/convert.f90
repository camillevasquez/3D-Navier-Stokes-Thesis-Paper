...
!$$ READ IN, INTERPOLATE AND WRITE OUT SCALAR FIELD.
  nx2_test = 3*nx_test/2
  nz2_test = 3*nz_test/2
  ALLOCATE(scalar_slice(nx2_test,nz2_test), scalar_slice2(nx2,nz2), &
       tempx1(nx2_test+1), tempx2(nx2+1), tempy1(ny_test+1), tempy2(ny+1), &
       temp_xz_scalar(nx2,nz2_test+1), tempz1(nz2_test+1), tempz2(nz2+1), &
       scalar_xz_interp(nx2,nz2,ny_test+1),scalar(nx2,nz2,ny+1), STAT=ierr)
  DO i = 1,nscalar
...
!$$ NEW GRID AND STORE.
     DO k = 1,ny_test+1
        ...
        DO z = 1,3*nz_test/2
           tempx1(1:nx2_test) = scalar_slice(1:nx2_test,z)
           tempx1(nx2_test+1) = scalar_slice(1,z)
           CALL interpolate_real(nx2_test+1,nx2+1,tempx1,tempx2)
           temp_xz_scalar(1:nx2,z) = tempx2(1:nx2)
        END DO
        temp_xz_scalar(1:nx2,nz2_test+1) = temp_xz_scalar(1:nx2,1)
        
        ! INTERPOLATE IN Z-DIRECTION
        DO x = 1,nx2
           tempz1 = temp_xz_scalar(x,1:nz2_test+1)
           CALL interpolate_real(nz2_test+1,nz2+1,tempz1,tempz2)
           scalar_xz_interp(x,1:nz2,k) = tempz2(1:nz2)
        END DO
     END DO

!$$ TAKE INTERPOLATED XZ-PLANES AND INTERPOLATE ONTO CORRECT NEW Y-LOCATIONS.
     DO z = 1,nz2
        DO x = 1,nx2
           tempy1 = scalar_xz_interp(x,z,1:ny_test+1)
           CALL interpolate_real(ny_test+1,ny+1,tempy1,tempy2)
           scalar(x,z,1:ny+1) = tempy2
        END DO
     END DO

!$$ OUTPUT INDIVIDUAL SLICES TO A FILE.
     DO k = 1,ny+1
        scalar_slice2 = scalar(1:nx2,1:nz2,k)
